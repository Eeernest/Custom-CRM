...
class AuthService:
  ...
  async def _authenticate_user(self, username: str, password: str) -> User | None:
    user_obj = await self.db_repo.get_by_username(username)
  ...
  async def login(self, username: str, password: str) -> TokenBase:
    user_obj = await self._authenticate_user(username, password)
    ...
    access_token = await self._create_access_token(
      data={"sub": str(user_obj.id), "role": user_obj.user_role},
